...
#	colors=['k','m']
	...
	difference=blackbody(x,39.8417,0.5558)-polynomial(x,35.1424,0.006119,2.0629)


	pl.plot(x,difference,color='b',linestyle='-', label='Sq3')#,label = j+': a = %.4f,b = %.4f'%(a,b))

#	pl.figtext(.15,.30, r'Difference = $a-10^{12}\frac{b}{2}\int^{\nu_1}_{\nu_0}\frac{h\nu}{e^{h\nu/{kT}}-1}d\nu-(a - cT^n)$',fontsize=18)#  ,  $\frac{\chi^2}{DOF} = %.3f$'%red_chi_sq ,fontsize=22)
...
#	pl.figtext(.15,.15, r'$P_{bias} = a - cT^n$',fontsize=18)#  ,  $\frac{\chi^2}{DOF} = %.3f$'%red_chi_sq ,fontsize=18)
	...
	pl.legend(loc='best')
        pl.title('Power difference between the fit curves')
	pl.grid()
