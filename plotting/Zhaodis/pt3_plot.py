...
def plot_cl_pt(sq,chan=1,r_frac = .80, parRes = {'Sq3': .1,'Sq4':.1},legends=True):
	...
	path = '/home/cryo/Data/output/20140511_cooldown/below50K' # Directory containing labeled data folders, e.g. .../20140426_162036_IV_08.51K
	item=dict()
...
#	  red_chi_sq=(sum((pR-fit_function(Tcl,a,c,n))**2))/(len(pR)-3)
	  ...
	  pl.plot(x,fit,color=colors[i],linestyle='-',label = j+': a = %.4f,b = %.4f'%(a,b))

	pl.figtext(.15,.35, r'$P_{bias} = a-10^{12}\frac{b}{2}\int^{\nu_1}_{\nu_0}\frac{h\nu}{e^{h\nu/{kT}}-1}d\nu$',fontsize=18)#  ,  $\frac{\chi^2}{DOF} = %.3f$'%red_chi_sq ,fontsize=22)
