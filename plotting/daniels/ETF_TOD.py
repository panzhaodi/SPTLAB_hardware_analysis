...
# My commented version of analyzeEtf.py I used to plot TODs and their FFts to investigate noisy ETFs.
...
def get_etf_data_points(data,SQ,ch,directory,rFrac,plot=True):								# This is the section I really need to look at
    '''
    this function takes the fft of the raw etf data and finds the amplitude of the 
       24Hz bin as well as an estimate on that amplitude and returns a bunch of stuff, 			#24 Hz bin = where we demodulate the signal
    ...
    if plot:
        pl.figure()
        pl.clf()
	new_dir=directory+'/%s_%s'%(SQ,rFrac['Ch1'])
	os.makedirs(new_dir) 

    for i,_tfreq in enumerate(tickle_freq):						# Loops through the tickle frequencies
        try:
        ...
        FQ = pl.fftfreq(n_pts_in_dat_i,1/Fs)[:n_pts_in_dat_i/2]				# What is this?
        FT = abs(pl.fft(dat_i*hanning_window))[:n_pts_in_dat_i/2]			# What is this?
        pkind = pl.argmin(abs(FQ-24))

#        max_amp = FT[pkind]
#        pl.find(FT>0.1*max_amp)

        pkl_quarter_inds = pl.find(abs(FQ-24)<0.25)					# Defining the 24Hz bin, little arbitrarily
        if plot:
	    pl.subplot(2,1,2)
	    pl.plot(pl.arange(n_pts_in_dat_i),dat_i)
	    pl.title('%s %s TOD'%(SQ,rFrac['Ch1']))
	    pl.subplot(2,1,1)
            pl.plot(FQ-24,FT,'.-',label=str(_tfreq),alpha=0.4)							# Plots the lines
	    pl.legend()
            pl.plot(FQ[pkl_quarter_inds]-24,FT[pkl_quarter_inds],'o',color = pl.gca().lines[-1].get_color())	# Plots points within the 24Hz bin
	    pl.title('%s %s FFT'%(SQ,rFrac['Ch1']))
	    pl.suptitle('%s'%directory)
	    if i < 10:
	        save_name = new_dir+'/0%s.png'%i
	    else:
		save_name = new_dir+'/%s.png'%i
...
def fit_data(fit_type,freqs,amps,errs,								# Won't need this for individual timestreams
             f_3db=10.0,
             gamma = 300,
             tau_0 = 60E-3,
             Rnow = 0,
...
def make_plots(data,savePlots=True,directory='',
               ...
               fit_freq_low_cut = None,								# Won't need this for individual timestreams
               inductance = 0,
               Rt = 0,
               gamma = 200,
               tau_0 = 60E-3,
    ...
    for i,bolo in enumerate(data.keys()):
        ...
        for _nrfrac, rfrac in enumerate(sorted(_now.keys(),reverse=True)):
            ...
            for _freq in np.sort(_now[rfrac]['tickle_frequencies'].keys()):
                ...
                responses.append(_now_tick['final_value'])	#				These two values go into computing
                errors.append(_now_tick['value_error'])		#				the error bars	
...
#            start_ind = valids.index(True)# this is the first 'valid' point
            ...
            errnorm = pl.asarray(errors)/responses[start_ind]	#				The error bars are set here
...
def analyze_etf_dir(directory, save=False, note='',						# This is the function I end up calling.
                    fit_model = 'single_body',
                    fit_freq_high_cut = None,
    ...
    fit_model options are:
         ['single_body', 'single_body_lr', 'two_body', 'two_body_lr']

    INPUTS:
        directory:    (str) This is the full path name of the directory that contians
                       the ETF (Lueker tickle) data you want to analyze
        
        save [False]: (bool) If True, plots will be saved to 'directory'

        note ['']:    (str) A note to be appended to the saved plot filenames

        fit_model ['single_body']: (str) This describes the fit to be done to the data. 
                      Appropriate options are:          
                      ['single_body', 'single_body_lr', 'two_body', 'two_body_lr']

        fit_freq_high_cut [None]: (int or float) Data points above this frequency will
                      not be included in the fit

        fit_freq_low_cut [None]: (int or float) Data points below this frequency will
                      not be included in the fit. The data will also be normalized at 
                      the data point frequency just above this fit_freq_low_cut.

        inductance [20E-6]: (int or float) In units of Henries this is the value of the 
                      inductance that the model will use. This only matters with models
                      that have the L/R ('lr') filter taken into account. This value is 
                      not fit for, it is held constant during the fit.

        gamma [200]: (int or float) This is the ratio of the intrinsic detector G 
                      (thermal conductance) to that of the bling. This is a starting 
                      guess for the fitter. 
        
        tau_0 [70E-3]: (int or float) This is the intrinsic time constant of the 
                      detector. This number will be used as the starting guess
                      for the fitter. If tau_0_range is given this value will be 
                      overwritten with the centroid of tau_0_range. 

        tau_0_range [None]: (list of 2 numbers) If given, this restricts the values
                     tau_0 the fit can explore. 

        Rt [1.1]: (int or float) This is the resistance of the entire LRC arm of the
                     detector. It is used with models that take into account the L/R ('lr')
                     filter. This value is not fit for, it is held constant during the fit,
                     but does change between depths in the transition as Rfrac*Rt. 
    OUTPUTS:
        None
...
#        tune_pkls = glob('%s/%s*_TuneBoloCombDan_*.pkl'%(directory,_SQ))
        ...
        for (_time,_pkl) in taus:
            ...
            rFrac,success = get_Rfrac_from_tune_log(tune_logs[tunes_index],_SQ,directory)	# Defined at line 354
            if not success:
                break
            # open the etf pickle and fit 
            thisFL = pkl.load(open(_pkl))
            for ch in [x for x in thisFL.keys() if 'Ch' in x]:
                data_points,err,outDict = get_etf_data_points(thisFL[ch],_SQ,ch,directory,rFrac)		# Defined at line 24
                try:
