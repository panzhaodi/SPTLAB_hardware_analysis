...
# Mangled version of analyzeETF that I used to plot very noisy ETF data.
...
#        pl.find(FT>0.1*max_amp)
	    ...
	    if i < 10:
	        save_name = '/home/cryo/Data/plots/test/Sq2_0%s.png'%i
	    else:
		save_name = '/home/cryo/Data/plots/test/Sq2_%s.png'%i
...
def make_plots(data,savePlots=True,directory='',
    ...
    for i,bolo in enumerate(data.keys()):
        ...
        if plot_residual:
            pl.subplot(121)
        pl.plot(1,1,'k',linestyle='none',label= '   %s   %s'%(r'$\frac{R_{TES}}{Rn}$',r'$f_{3db}\,(Hz)$'))
...
#            start_ind = valids.index(True)# this is the first 'valid' point
            ...
            if 1/(2*pl.pi*tau) < 500 and fit_success==True:
                label = '%8.3f  %2.0f'%(float(rfrac),float(f_3db))
            else:
                label = '%8.3f  %2.0f'%(float(rfrac),float(f_3db))
#            print label,'+/-', tau_err*1000

            # plot the data
            pl.plot(freqs,respnorm,'o-',label=label,
...
#                pl.loglog(freqs_plot,model_to_plot,color=pl.gca().lines[-1].get_color(),linewidth=0.4)#,label = label)
	    ...
	    pl.plot(pl.array([f_3db,f_3db]),pl.array([.001,10]),label='%8.3f f_3db'%(float(rfrac)))
        ...
        pl.title('Quick Look ETF (%s),Rn=%.fmOhms'%(bolo,Rt*1E3))
