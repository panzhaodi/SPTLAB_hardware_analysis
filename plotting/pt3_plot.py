...
def plot_stage_pt(sq,chan=1,temp='???',directory = '/home/cryo/Data/output/20140511_cooldown',parRes=0.1,legends=False):
...
def plot_cl_pt(sq,chan=1,r_frac = .95, parRes = {'Sq2':.1, 'Sq3': .1,'Sq4':.1},directory = '/home/cryo/Data/output/20140511_cooldown/',legends=True,do_fit=True,cutoff=20):
...
#	colors=['k','m']
	...
	pl.figure(2)
  	  ...
  	  pkls = glob(directory+'/*_??.??K/'+j+'_TuneBoloCombDan*.pkl')
  	  if len(pkls)==0:
  	      print "No matching .pkl file found."
  	      return
  	  for _pkl in pkls:
	  ...
	  for _run in item:
            ...
            #ind=pl.argmin(vals['i'])
	      if vals['hasPTurn']:
		if float(_run.rpartition('_')[-1].partition('K')[0]) < cutoff: # In case you only want to plot certain temp ranges.
...
def plot_cl_ptime(sq,temp=20,r_frac = .80, parRes = {'Sq3': .1,'Sq4':.1},directory = '/home/cryo/Data/output/20140511_cooldown',legends=True):
	...
	pl.figure(1)
        ...
        for i,j in enumerate(sq):
	  item=dict()
  	  pkls = glob(directory+'/20140618*min'+str(temp)+'*/'+j+'_TuneBoloCombDan*.pkl')
  	  if len(pkls)==0:
  	      print "No matching .pkl file found."
  	      return
  	  for _pkl in pkls:
	...
	pl.xlabel('Minutes elapsed')
        pl.ylabel('Bias Power on TES [pW]')
	pl.title('Stage at 290mK, Coldload at %sK'%temp)
	if legends:        
	    pl.legend()
	pl.grid(b=True)
