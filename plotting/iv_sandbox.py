## I'm using this file to experiment with changes to iv_plot without changing the working module ##
...
def plotIv(given, figure=1,note='',parRes=0.03,
    ...
    try:
        ...
        if pkls:
            for _pkl in pkls:
                sq.add(_pkl.rpartition('/')[-1].partition('_')[0])
                _label = _pkl.rpartition('/')[0].rpartition('_')[-1]+'_'+_pkl.rpartition('/')[-1].partition('_')[0]
    ...
    for _n_sq,_sq in enumerate(sorted(item.keys())):
        if plot_by_squid:
            pl.figure(figure+sorted(sq).index(_sq.partition('_')[-1]))
##            pl.clf()
        _it = item[_sq][0]
        data[_sq] = dict()
        ...
        for _n_chan, chan in enumerate(sorted(_it['data'].keys())):
            ...
            if plot_by_bolo: # plot_by_bolo will skip right to here for plotting
                ...
                pl.title(_sq.partition('_')[-1]+'_'+str(chan))
            ...
            if flip_axes:
               pl.plot(vals['r'],vals['p'],'.-',label=_sq+'_'+str(chan))
               pl.legend(loc=1)
            else:
...
#                vals['smooth_loopgain'] = np.polyfit(vals['p'],vals['loopgain'],2)
    ...
    if returnData:
        return outDict
####################################################################################################################
def plot_stageIv(given, figure=1,note='',parRes=0,
           plot_iv = True,
           plot_loopgain = False,
           plot_by_squid = True,
           plot_by_bolo = False,
    ...
    try:
        ...
        pkls = glob(given+'/*_stage/*TuneBoloCombDan*.pkl')
        if pkls:
            for _pkl in pkls:
                sq.add(_pkl.rpartition('/')[-1].partition('_')[0])
                _label = _pkl.partition('_stage')[0].rpartition('_')[-1]+'_'+_pkl.rpartition('/')[-1].partition('_')[0]
                item[_label] = [pkl.load(open(_pkl,'r'))]
        else:
            print 'reading pickle'
            # if it does not have keys or pkls in it, 
