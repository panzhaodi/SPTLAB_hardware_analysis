...
def two_pole_fit(w,gamma,tau,tau_0):
#    tau_e = 2.0*inductance/(Rfrac*Rn)

    model = 1.0/( (1+imag*w*tau_0)(1+gamma)/(1+gamma+imag*w*tau_0) + tau/tau_0 -1)
    
    norm_model = model*( (1+imag*w[0]*tau_0)(1+gamma)/(1+gamma+imag*w[0]*tau_0) + tau/tau_0 -1)
...
def investigate_tau(data, savePlots=False,SqCh=None,Rfrac=None,
                    fit_freq_cutoff = 1000,extraTaus = []):
...
#                pl.loglog(freqs,respnorm,'.',markersize=12,label=label,color = pl.cm.gist_rainbow(float(_nrfrac)/rfrac_count))
                pl.errorbar(freqs,respnorm,yerr=errnorm,fmt = '.',label=label,color = pl.cm.gist_rainbow(float(_nrfrac)/rfrac_count))
            # only plot the fit line to the frequencies we fit to
                freqs_plot = pl.linspace(freqs[0],freqs[ind_to_fit],500)# a dummy list to plot a smooth fit line
            #freqs_plot = pl.linspace(freqs[0],freqs[-1],500)# To plot fit across all freqs
                pl.loglog(freqs_plot,single_pole(2*pl.pi*freqs_plot,tau),color=pl.gca().lines[-1].get_color(),linewidth=0.8)
                colors = ['b','g','m','k','y','b','g','m','k','g','y','b','g','m','k','g','y']
                colors = colors[:len(extraTaus)]
                for _etau,_color in zip(extraTaus,colors):
