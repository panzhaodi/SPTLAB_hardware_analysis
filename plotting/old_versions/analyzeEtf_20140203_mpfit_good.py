...
def get_etf_data_points(data,SQ,ch,nchuncks=4):
    '''
    ...
    SQ = squid
    ch = channel 
    nchuncks[4] = the number of chucks to split the fft into to get noise
    '''
    ...
    for i,_tfreq in enumerate(tickle_freq):
        try:
            ...
            break
#        print asdf
#        # Split the slice into chunks.
#        n_pts_per_chunk = len(dat_i)/nchunks
#        n_pts_per_chunk -= n_pts_per_chunk%2 # this just makes the length of the chunks to be even
#        chunks = [ slice(ichunk*n_pts_per_chunk, (ichunk+1)*n_pts_per_chunk)
#                   for ichunk in range(nchunks) ]
#        
#        frequencies = np.fft.fftfreq(n_pts_per_chunk, 1./data.telescope.receiver.samplerate)
...
#        pl.find(FT>0.1*max_amp)
        ...
        if len(pkl_quarter_inds)>3:
            thisDat['%.1f'%_tfreq] = sum(FT[pkl_quarter_inds])
            thisErr['%.1f'%_tfreq] = (sum(FT[pkl_quarter_inds+len(pkl_quarter_inds)])+
                                 sum(FT[pkl_quarter_inds-len(pkl_quarter_inds)]))/2
        else:
            thisDat['%.1f'%_tfreq] = sum(FT[(pkind-1):(pkind+2)])
            thisErr['%.1f'%_tfreq] = (sum(FT[pkind+3:pkind+6])+
...
#    print asdf
    ...
    return tau, tau_err, gamma, gamma_err, tau_0, tau_0_err,Rnow,Rnow_err,inductance,inductance_err,chi2_reduced


def make_plots(data,savePlots=True,directory='',
               note='',
               fit_model = 'single_body',
               fit_freq_cutoff = None,
               inductance = 0,
               Rt = 0,
               gamma = 200,
               tau_0 = 60E-3,
    ...
    for i,bolo in enumerate(data.keys()):
        ...
        for _nrfrac, rfrac in enumerate(sorted(_now.keys(),reverse=True)):
            ...
            # only fit up to a specified frequency if fit_freq_cutoff is supplied
            if fit_freq_cutoff:
                print 'Only fitting frequencies below %s.'%(str(fit_freq_cutoff))
                ind_to_fit = pl.find(freqs < fit_freq_cutoff)[-1]
                pl.axvline(fit_freq_cutoff,color='k',alpha=0.3)#linestyle='--'
            else:
                ind_to_fit = -1

            # fit the data
            print '---------------------------------Now fitting rfrac = '+str(rfrac)
            fit_output = fit_data(fit_model,freqs[:ind_to_fit],
                                  respnorm[:ind_to_fit],
            ...
            tau,tau_err,gamma,gamma_err,tau_0,tau_0_err,Rnow,Rnow_err,inductance,inductance_err,chi2_reduced = fit_output
...
#                        color = pl.cm.gist_rainbow(float(_nrfrac)/rfrac_count))
            ...
            if min_amp_to_plot>min(respnorm): min_amp_to_plot = min(respnorm)
        ...
        pl.title('Quick Look ETF (%s),  L=%.2uH  Rn=%.fmOhms'%(bolo,inductance*1E6,Rt*1E3))
