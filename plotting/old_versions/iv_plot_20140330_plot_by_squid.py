...
def plotIv(given, figure=1,note='',parRes=0.03,
    ...
    # no matter what we will use figure(figure), so we will initiate it
    #    and clear it now
    pl.figure(figure)
    pl.clf()
    data = dict()
    max_n_sq = len(item)
    ordered_squids = sorted(item.keys())
    for _n_sq,_sq in enumerate(ordered_squids):
        if plot_by_squid:
        ...
        if plot_by_squid or _n_sq == max_n_sq:
            if plot_iv:
