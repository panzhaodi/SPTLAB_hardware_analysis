...
def plotIv(given,sq=[],parRes=0.03,plot_iv=True,plot_loopgain=True,legends=False,return_data=False):
	...
	if len(sq)==0:
	  ...
	  for _sq in sq:
		pkls=glob(given+'/'+(_sq)+'_TuneBoloCombDan*.pkl')
		if len(pkls)==0:
	  	  print "No matching .pkl file found."
	  	  return	
		item[_sq] = [pkl.load(open(pkls[0],'r'))]
