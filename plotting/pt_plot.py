...
def plot_stable_pt(sq,chan=1,r_frac = .80, parRes = .25,directory = '/home/cryo/Data/output/20141230_cooldown',legends=True,do_fit=True,cutoff=50, use_sim=False, do_average=False):
	...
	#  return pl.array(ans)
        ...
        if use_sim:
          data = np.loadtxt('donnasim220ghz.s2p')
           #  Frequency Sonnet
          f_90 = data[:,0]*1e9
           # S21 mag Sonnet
          s_90 = data[:,5]**2
	  ...
	  def fit_function(T,a,b):
            ans=[] 
	    for _num in T:
	      ans.append(a-b*1e12*quad(lambda x:((6.626e-34)*x)*simfunc(x)/(pl.e**(((4.8e-11)*x)/_num)-1),150e9,300e9)[0])
	    return pl.array(ans)           
	...
	pl.figure(2)
	  ...
	  if do_fit:
	      ...
	      pl.plot(x,fit,color=colors[i],linestyle='-',label = '220GHz'+': a = %.3f, b = %.3f $\pm$ %.3f'%(a,b,pl.sqrt(parameters[1][1,1])))
