...
# I am hardcoding the location of the bias correction factor!!!!!!!!!!!!!!!!!1
bias_correction_factor = pkl.load(open('/home/cryo/Code/uofc/tools/fft_noise_bias_correction_factor_theory.pkl','r'))
...
#        pl.find(FT>0.1*max_amp)
        ...
        #####       the S/N measured
        S_N = now_sig/now_err
        thisDat['%.1f'%_tfreq] = now_sig*get_bias_correction_factor(S_N)
        print S_N, get_bias_correction_factor(S_N)
...
def get_bias_correction_factor(S_N):
    S_Ns = bias_correction_factor['S/N']
    CorFac = bias_correction_factor['Correction_factor']
    if S_N > S_Ns[-1]:
        return 1.0
    else:
        return scipy.interp(S_N, S_Ns, CorFac)
...
#                        color = pl.cm.gist_rainbow(float(_nrfrac)/rfrac_count))
            ...
            if fit_success:
                ...
                if inductance > 1E-6:
                    pl.loglog(freqs_plot,abs(1./(1+imag*(2*pl.pi*freqs_plot)*(2*inductance/Rnow))),'-k')
                if plot_residual:
                    pl.subplot(122)
...
def analyze_etf_dir(directory, save=False, note='',
                    fit_model = 'single_body',
                    fit_freq_high_cut = None,
                    fit_freq_low_cut = None,
                    inductance = 20E-6,
    ...
    INPUTS:
        ...
        Rt [0.261]: (int or float) This is the resistance of the entire LRC arm of the
